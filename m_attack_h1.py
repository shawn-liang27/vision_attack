"""Stage 18: H1 test -- whole-image M-Attack loss, ROI-masked UPDATE.

The sharp discriminator between "localized camouflage is salvageable" and
"concealment fundamentally needs broad context". Difference from m_attack_roi
(which confined the crops -> local signal): here the crop-matching loss is
WHOLE-IMAGE (so the representation matches what LLaVA reads globally), we
backprop the full gradient, then ZERO the gradient outside the ROI before the
MI-FGSM step. Perturbation lives only in the ROI(+padding); the signal driving
it is computed globally. "Mask the update, not the loss."

Swept over padding of the perturbation region (tight ROI -> whole image), single
surrogate = LLaVA-1.5's encoder. Compare to m_attack_roi.py (local signal) at
matched padding: if H1 flips LLaVA where the local-signal version didn't, the
signal locality was the bottleneck, not the perturbation region.
...
def main():
    ...
    ap.add_argument("--pads", default="0,0.05,0.15,0.35,1.0",
                    help="padding fractions of RES for the PERTURBATION region (1.0=whole image)")
    ap.add_argument("--object", default="dog")
    ap.add_argument("--outdir", default="results/m_attack_h1")
    args = ap.parse_args()
    RES = args.input_res
    ...
    print(f"H1 (global loss, ROI-masked update)  surrogate={args.model}  "
          f"{args.source}->{args.target}  eps={args.eps}/255")
    ...
    print(f"baseline: cos(orig,target)={sim0:.3f} global P({args.object})={p0[0]:.3f}")

    pads = [float(p) for p in args.pads.split(",")]
    rows = []
    summary = [f"H1 global-loss/ROI-update  {args.source}->{args.target} eps={args.eps}/255 steps={args.steps}",
               f"baseline cos(orig,target)={sim0:.4f} global P({args.object})={p0[0]:.4f}"]
    for frac in pads:
        pad = int(frac * RES)
        ...
        for step in range(args.steps):
            with torch.no_grad():
                tgt_feat = embed(crop(x_tgt))            # WHOLE-image target crop
            adv = torch.clamp(x0 + delta * region, 0, 1)
            src_feat = embed(crop(adv))                  # WHOLE-image source crop (global loss)
            sim = (src_feat * tgt_feat).sum()
            g, = torch.autograd.grad(sim, delta)
            with torch.no_grad():
                g = g * region                          # <-- mask the UPDATE, not the loss
        ...
        save(adv, f"{args.outdir}/pad{frac:g}.png")
        rows.append(dict(frac=frac, coverage=coverage, gsim=gsim,
                         p_obj=float(pv[0]), p_couch=float(pv[2]), argmax=labels[int(pv.argmax())]))
        line = (f"pad={frac:<5g} region={coverage*100:4.1f}%  cos(adv,target)={gsim:.3f}  "
    ...
    ax[0].set_xlabel("perturbation region coverage (%)"); ax[0].set_ylabel("global zero-shot P")
    ax[0].set_title("H1 dose-response (global signal, ROI update)"); ax[0].legend()
    ax[1].plot(cov, [r["gsim"] for r in rows], "o-", color="tab:green")
    ax[1].axhline(sim0, ls=":", c="gray")
    ax[1].set_xlabel("perturbation region coverage (%)"); ax[1].set_ylabel("cos(adv, target)")
    ax[1].set_title("global steering achieved vs region size")
    fig.suptitle("H1: whole-image M-Attack loss + ROI-masked update (LLaVA-1.5 encoder)", fontsize=12)
    fig.tight_layout(); fig.savefig(f"{args.outdir}/h1_dose_response.png", dpi=130)
    print(f"\nsaved {args.outdir}/ -> caption all via vlm_eval; compare to m_attack_roi (local signal)")
